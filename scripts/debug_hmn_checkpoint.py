#!/usr/bin/env python
"""
Debug tool for Humanoid (HMN) policy checkpoints, with Rainbow robot defaults.

Author: Mariano

This script allows debugging and investigating Pi0 Humanoid policy checkpoints:
...
Note: For Rainbow robot, we use Pi0FASTConfig because all Rainbow models
are trained with this configuration, even though this script is for 
Humanoid policy debugging.

Usage:
  python scripts/debug_hmn_checkpoint.py --checkpoint_path [CHECKPOINT_PATH] --action_dim [DIM] --action_horizon [HORIZON]

Example for Rainbow robot:
  python scripts/debug_hmn_checkpoint.py --checkpoint_path ./checkpoints/pi0_fast_rainbow/my_exp/10000/params --gripper_indices 7,15 --analyze_tokens --plot_actions
...
def load_checkpoint(checkpoint_path: str, action_dim: int, action_horizon: int, max_token_len: int):
    """Load the Pi0 model from checkpoint for Humanoid policy debugging.
    
    Note: For Rainbow robot, we use Pi0FASTConfig because all Rainbow models
    are trained with this configuration, even though we're renaming the script
    to reflect Humanoid policy debugging.
    """
    logger.info(f"Loading checkpoint from {checkpoint_path}")
    
    # Create model config - using Pi0FASTConfig for Rainbow robot
